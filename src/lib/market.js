// src/lib/market.js
// Matching & ranking engine for the Sovereign Grid demo (Wave 1).
...
// transaction-specific route gate — never a blanket geography exclusion (SPEC §7.1, §10).
 ...
 *   approved    -> all baseline + transaction-specific evidence present; route is bookable.
 *   conditional -> attractive but a route-specific requirement is not yet evidenced;
 *                  surfaced to the buyer as DISQUALIFIED-by-policy (not bookable).
 *   hold        -> no/failed route baseline evidence; excluded from the match set.
 *   not_applicable -> not a China route; gate does not apply.
 */
 ...
 * category is used so a policy disqualification can be surfaced separately
 * (as an attractive-but-not-bookable offer) rather than silently dropped.
 *   'policy'  -> sovereign eligibility / trade-control / China route gate / data residency
 *   'location'-> jurisdiction mismatch
 *   'firmness'-> access-model mismatch (firm vs interruptible)
 *   'capacity'-> below minimum capacity
 *   'timing'  -> start-date / min-term mismatch
 *   'compat'  -> accelerator / software / portability mismatch
 */
export function hardFilter(listing, request) {
  ...
  // Data residency (policy category) — a hard requirement for PRIMARY-region
  // deployments. Failover-region nodes may legitimately hold a different
  // residency (e.g. Project Falcon: EU primary + UAE failover), so residency is
  // only enforced for listings in the buyer's primary jurisdictions (sovereignty
 ...
 * breakdown: { <dimension>: { weight, score, contribution } }.
 */
 ...
 *   offers      -> bookable matches, sorted by score desc.
 *   policyHolds -> attractive offers blocked by a route/policy condition (not bookable),
 *                  sorted by score desc, each with disqualifyReason.
 *   excluded    -> everything else that failed a hard filter, with reasons.
 */
