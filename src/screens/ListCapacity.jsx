...
const FIRMNESS_LABELS = { firm: 'Firm (committed)', interruptible: 'Interruptible-only' }
...
export default function ListCapacity() {
  ...
  const onAccel = (model) => {
    ...
    setF((p) => ({ ...p, accel: hit ? hit : p.accel }))
  }
  const onCountry = (c) => setF((p) => ({ ...p, country: c }))
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <div className="mb-2 flex items-center gap-2">
        <DemoBadge />
        <span className="text-xs text-text-3">Seller listing appears in the marketplace. Net payout is a demo preview.</span>
      </div>
      <h1 className="sg-display text-2xl">List Capacity</h1>
      <p className="mt-1 text-sm text-text-2">Declare capacity inventory and see the expected net payout before submitting.</p>

      <form onSubmit={submit} className="mt-6 space-y-5">
        <section className="sg-card p-5">
          <h2 className="mb-3 text-sm font-semibold text-text-2">Listing identity</h2>
        ...
        <section className="sg-card p-5">
          <h2 className="mb-3 text-sm font-semibold text-text-2">Inventory</h2>
          <div className="grid gap-4 sm:grid-cols-3">
            <Field label="Accelerator">
              <Select
                value={acceleratorProfiles[f.accel]?.model}
                onChange={onAccel}
            ...
            <Field label="Facility country">
              <Select value={f.country} onChange={onCountry} options={COUNTRIES} label="Facility country" />
            </Field>
        ...
        <section className="sg-card p-5">
          <h2 className="mb-3 text-sm font-semibold text-text-2">Price & evidence</h2>
          <div className="grid gap-4 sm:grid-cols-3">
        ...
        {/* Net payout preview */}
        <section className="rounded-lg border border-[color:var(--sg-border)] bg-elevated p-5">
          <div className="mb-2 flex items-center gap-2">
            <h2 className="text-sm font-semibold text-text-2">Net payout preview</h2>
...
function PayoutRow({ label, value, accent }) {
  return (
    <div className={`rounded-md border p-3 ${accent ? 'border-[color:var(--sg-accent)] bg-[color:var(--sg-accent-dim)]' : 'border-[color:var(--sg-border)] bg-card'}`}>
      <div className="text-[11px] uppercase tracking-wide text-text-3">{label}</div>
      <div className={`sg-num text-lg font-semibold ${accent ? 'text-accent' : 'text-text-1'}`}>{value}</div>
    </div>
  )
}
