...
## Wave N — party lifecycle + transacted auto-observation (2026-09-26)
- POST /api/deals/:id/transitions (party-driven; LEGAL_TRANSITIONS matrix; audit via new SD fn audit_log_transition) + auto-observation on `contracted` (SD fn deal_contract_observation → market_observations, source='transacted', ref 'deal:<id>', deduped by 0007 partial-unique index). 0010 additive — new fn names only (0009 lesson applied).
